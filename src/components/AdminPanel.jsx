...
const t = {
    en: {
        ...
        categoryLabel: "Categorization",
        selectMain: "Select Main Category...",
        selectSub: "Select Sub (Optional)...",
        nameEn: "Name (English)",
        nameAr: "Name (Arabic)",
    ...
    ar: {
        ...
        selectMain: "اختر الصنف الرئيسي...",
        selectSub: "اختر صنف فرعي (اختياري)...",
        ...
        newCat: "صنف جديد (إنجليزي)",
        newCatAr: "صنف جديد (عربي)",
        total: "المجموع",
...
const AdminPanel = ({ restaurant, onLogout }) => {
  ...
  const [newSub, setNewSub] = useState({ catId: '', en: '', ar: '' });
  const [editingCatId, setEditingCatId] = useState(null);
  const [editCatData, setEditCatData] = useState({ en: '', ar: '' });
  ...
  async function fetchCategories() {
    const { data } = await supabase.from('categories').select(`*, subcategories(*)`).eq('restaurant_id', restaurant.id).order('created_at');
    setCategories(data || []);
  }
  ...
  const handleImageUpload = async (e) => {
    ...
    if (file) {
      ...
      try {
        ...
        setStatus('');
      } catch (err) { setStatus('❌ Image Error'); }
    }
  };
  ...
  const handleSaveItem = async (e) => {
    e.preventDefault();
    setStatus(t[lang].save + '...');
    
    if (!newItem.category_id) { setStatus('❌ ' + t[lang].selectMain); return; }
    ...
    const payload = {
      ...
      subcategory_id: newItem.subcategory_id || null, // FIX: Send null if empty string
    ...
    let error;
    if (editingItem) {
        const { error: err } = await supabase.from('menu_items').update(payload).eq('id', editingItem);
        error = err;
    } else {
  ...
  const toggleAvailability = async (item) => {
      ...
      const updatedItems = menuItems.map(i => i.id === item.id ? {...i, is_available: newStatus} : i);
  ...
  const handleAddCategory = async () => {
    if (!newCat.en) return;
    await supabase.from('categories').insert({ restaurant_id: restaurant.id, name_en: newCat.en, name_ar: newCat.ar });
    setNewCat({ en: '', ar: '' });
    fetchCategories();
  };
  ...
  const handleSaveEditCat = async () => {
    ...
    await supabase.from('categories').update({ name_en: editCatData.en, name_ar: editCatData.ar }).eq('id', editingCatId);
  ...
  const handleAddSub = async (catId) => {
    ...
    await supabase.from('subcategories').insert({ category_id: catId, name_en: newSub.en, name_ar: newSub.ar });
    setNewSub({ catId: '', en: '', ar: '' });
    fetchCategories();
  };
    ...
    <div dir={lang === 'ar' ? 'rtl' : 'ltr'} className={`min-h-screen font-sans transition-colors duration-300 ${bgMain} ${textMain} selection:bg-[#3c3728] selection:text-[#ebe3c6]`}>
      ...
      <div className={`sticky top-0 z-40 px-6 md:px-8 py-5 flex flex-col md:flex-row justify-between items-center gap-4 transition-all duration-300 backdrop-blur-md ${isDark ? 'bg-[#1E1E1E]/80 border-[#333]' : 'bg-white/80 border-[#3c3728]/5'} border-b`}>
        <div className="flex items-center gap-4 w-full md:w-auto">
          <div className="w-12 h-12 bg-[#3c3728] rounded-2xl flex items-center justify-center text-[#ebe3c6] font-bold text-xl shadow-lg">
            {restaurant.name_en?.charAt(0) || 'R'}
          </div>
          <div>
            <h1 className="font-bold text-xl leading-tight">{restaurant.name_en}</h1>
            <p className={`text-xs font-bold uppercase tracking-widest ${textMuted}`}>{t[lang].dashboard}</p>
          </div>
        </div>
        
        <div className="flex items-center gap-3 w-full md:w-auto justify-end">
           {/* Language Switch */}
           <button onClick={() => setLang(lang === 'en' ? 'ar' : 'en')} className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold ${isDark ? 'bg-[#333] text-gray-300' : 'bg-gray-100 text-gray-600'}`}>
              <GlobeIcon /> {lang === 'en' ? 'العربية' : 'English'}
           </button>

           <button onClick={() => setIsDark(!isDark)} className={`p-2.5 rounded-xl transition-colors ${isDark ? 'bg-[#333] text-yellow-400' : 'bg-gray-100 text-gray-400 hover:text-orange-500'}`}>
              {isDark ? <SunIcon /> : <MoonIcon />}
           </button>

           <div className={`p-1.5 rounded-xl flex gap-1 ${isDark ? 'bg-[#2C2C2C]' : 'bg-[#ebe3c6]/30'}`}>
             {['items', 'categories'].map(tab => (
               <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 md:px-6 py-2 rounded-lg text-xs md:text-sm font-bold transition-all ${activeTab === tab ? `${isDark ? 'bg-[#3c3728] text-white' : 'bg-white text-[#3c3728]'} shadow-sm` : `${isDark ? 'text-gray-400' : 'text-[#3c3728]/60'}`}`}>
                 {t[lang][tab]}
               </button>
             ))}
           </div>
           
           <button onClick={onLogout} title={t[lang].logout} className="p-2.5 rounded-xl bg-red-50 text-red-400 hover:bg-red-100 transition-colors">
              <LogoutIcon />
           </button>
        </div>
      </div>
            ...
            <motion.div key="items" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-4 space-y-6 order-2 lg:order-1">
                 <div className={`p-6 md:p-8 rounded-[2rem] shadow-xl border sticky top-28 transition-colors ${bgCard} ${editingItem ? 'border-orange-500/30 ring-4 ring-orange-500/10' : borderMain}`}>
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-lg font-bold">{editingItem ? t[lang].editItem : t[lang].newItem}</h2>
                        {status && <span className={`text-[10px] font-bold px-3 py-1 rounded-full ${status.includes('Error') ? 'bg-red-500/10 text-red-500' : 'bg-green-500/10 text-green-500'}`}>{status}</span>}
                    </div>
                    ...
                    <form onSubmit={handleSaveItem} className="space-y-4">
                       ...
                       <div className={`p-4 rounded-xl border space-y-3 ${isDark ? 'bg-[#252525] border-[#333]' : 'bg-gray-50 border-gray-100'}`}>
                          ...
                          <select className={inputClass} value={newItem.category_id} onChange={e => setNewItem({...newItem, category_id: e.target.value, subcategory_id: ''})} required>
                             <option value="">{t[lang].selectMain}</option>
                             {categories.map(c => <option key={c.id} value={c.id}>{lang === 'en' ? c.name_en : c.name_ar}</option>)}
                          </select>
                          <select className={inputClass} value={newItem.subcategory_id} onChange={e => setNewItem({...newItem, subcategory_id: e.target.value})} disabled={!newItem.category_id}>
                             <option value="">{t[lang].selectSub}</option>
                             {categories.find(c => c.id === newItem.category_id)?.subcategories.map(s => <option key={s.id} value={s.id}>{lang === 'en' ? s.name_en : s.name_ar}</option>)}
                          </select>
                       </div>
                       ...
                       <div className="grid grid-cols-2 gap-2">
                           <textarea value={newItem.desc_en} onChange={e => setNewItem({...newItem, desc_en: e.target.value})} placeholder={t[lang].descEn} className={`${inputClass} h-20 resize-none text-xs`} dir="ltr" />
                           <textarea value={newItem.desc_ar} onChange={e => setNewItem({...newItem, desc_ar: e.target.value})} placeholder={t[lang].descAr} className={`${inputClass} h-20 resize-none text-xs`} dir="rtl" />
                       </div>

                       {/* Price & Image */}
                       <div className="flex gap-3">
                          <input value={newItem.price} onChange={e => setNewItem({...newItem, price: e.target.value})} placeholder={t[lang].price} className={`${inputClass} w-1/2 font-bold`} required />
                          <label className={`w-1/2 flex items-center justify-center border border-dashed rounded-xl cursor-pointer hover:border-[#3c3728] gap-2 text-xs font-bold transition-all ${isDark ? 'border-[#444] hover:bg-[#333]' : 'bg-gray-50'}`}>
                             <input type="file" hidden onChange={handleImageUpload} />
                             <UploadIcon /> {newItem.image_url ? t[lang].changeImg : t[lang].upload}
                          </label>
                       </div>
                       ...
                       <div className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer ${newItem.is_available ? (isDark ? 'bg-emerald-900/20 border-emerald-900/50' : 'bg-emerald-50 border-emerald-100') : (isDark ? 'bg-red-900/20 border-red-900/50' : 'bg-red-50 border-red-100')}`} onClick={() => setNewItem({...newItem, is_available: !newItem.is_available})}>
                            <div className={`w-5 h-5 rounded-full flex items-center justify-center border ${newItem.is_available ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-gray-400'}`}>
                                {newItem.is_available && <CheckIcon />}
                            </div>
                            <span className="text-xs font-bold uppercase">{newItem.is_available ? t[lang].available : t[lang].soldOut}</span>
                       </div>

                       <div className="pt-2 flex gap-3">
                          {editingItem && <button type="button" onClick={resetItemForm} className={`px-4 py-3 font-bold rounded-xl ${isDark ? 'bg-[#333]' : 'bg-gray-100'}`}>{t[lang].cancel}</button>}
                          <button className={`flex-1 py-3 font-bold rounded-xl text-white shadow-lg transition-all ${editingItem ? 'bg-orange-500' : 'bg-[#3c3728]'}`}>
                              {editingItem ? t[lang].update : t[lang].save}
                          </button>
                       </div>
                    </form>
                 </div>
              ...
              <div className="lg:col-span-8 space-y-6 order-1 lg:order-2">
                 ...
                 <div className="relative">
                     <div className={`absolute top-4 ${lang === 'ar' ? 'left-4' : 'right-4'} ${textMuted}`}><SearchIcon /></div>
                     <input 
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        placeholder={t[lang].search} 
                        className={`w-full p-4 ${lang === 'ar' ? 'pl-12' : 'pr-12'} rounded-2xl border shadow-sm outline-none ${bgCard} ${borderMain} ${textMain} focus:ring-2 focus:ring-[#3c3728]/20`} 
                     />
                 </div>

                 {loading ? (
                    <div className="text-center py-20 animate-pulse text-gray-400">{t[lang].loading}</div>
                 ) : (
                        ...
                        <motion.div layout key={item.id} className={`p-4 rounded-2xl shadow-sm border flex gap-4 items-start group hover:shadow-md transition-all ${bgCard} ${!item.is_available && 'opacity-60 grayscale'} ${editingItem === item.id ? 'border-orange-400 ring-2 ring-orange-500/20' : borderMain}`}>
                            ...
                            <div className="flex-1 min-w-0 py-1">
                                ...
                                <div className="flex items-center gap-2 mt-3">
                                    <button onClick={() => toggleAvailability(item)} className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors ${item.is_available ? 'bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20' : 'bg-red-500/10 text-red-500 hover:bg-red-500/20'}`}>
                                        {item.is_available ? <><EyeIcon /> {t[lang].available}</> : <><EyeOffIcon /> {t[lang].soldOut}</>}
                                    </button>
                 ...
                 {!loading && filteredItems.length === 0 && (
                     <div className={`text-center py-20 rounded-3xl border border-dashed ${bgCard} ${borderMain} ${textMuted}`}>{t[lang].noItems}</div>
                 )}
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="categories" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="max-w-4xl mx-auto space-y-6">
               <div className={`p-6 rounded-2xl shadow-lg border flex gap-4 items-end ${bgCard} ${borderMain}`}>
                  <div className="flex-1 space-y-1">
                      <label className={`text-[10px] font-bold uppercase tracking-widest px-1 ${textMuted}`}>{t[lang].newCat}</label>
                      <input value={newCat.en} onChange={e => setNewCat({...newCat, en: e.target.value})} className={inputClass} placeholder="e.g. Breakfast" dir="ltr" />
                  </div>
                  <div className="flex-1 space-y-1 text-right">
                      <label className={`text-[10px] font-bold uppercase tracking-widest px-1 ${textMuted}`}>{t[lang].newCatAr}</label>
                      <input value={newCat.ar} onChange={e => setNewCat({...newCat, ar: e.target.value})} className={`${inputClass}`} placeholder="مثال: فطور" dir="rtl" />
                  </div>
                    ...
                    <div key={cat.id} className={`rounded-3xl border shadow-sm overflow-hidden ${bgCard} ${borderMain}`}>
                       <div className={`p-5 flex justify-between items-center border-b ${isDark ? 'border-[#333] bg-[#2C2C2C]/50' : 'border-gray-50 bg-gray-50/50'}`}>
                          {editingCatId === cat.id ? (
                            ...
                            <>
                                <h3 className={`font-bold ${textMain}`}>{lang === 'en' ? cat.name_en : cat.name_ar} <span className="text-gray-500 text-xs">({lang === 'en' ? cat.name_ar : cat.name_en})</span></h3>
                                <div className="flex gap-1">
                                    <button onClick={() => { setEditingCatId(cat.id); setEditCatData({ en: cat.name_en, ar: cat.name_ar }) }} className={`p-2 rounded-lg ${textMuted} hover:bg-gray-100 hover:text-[#3c3728]`}><EditIcon /></button>
                                    <button onClick={() => handleDeleteCategory(cat.id)} className={`p-2 rounded-lg ${textMuted} hover:bg-red-500/10 hover:text-red-500`}><TrashIcon /></button>
                                </div>
                            </>
                          )}
                       </div>
                       ...
                       <div className="p-5 space-y-3">
                          {cat.subcategories.map(sub => (
                             <div key={sub.id} className={`flex justify-between items-center p-3 rounded-xl border border-transparent group/sub ${isDark ? 'bg-[#252525] hover:border-[#444]' : 'bg-gray-50 hover:border-gray-200'}`}>
                                <span className={`text-sm font-medium ${textMuted}`}>{lang === 'en' ? sub.name_en : sub.name_ar}</span>
                                <button onClick={() => handleDeleteCategory(sub.id)} className="text-gray-400 hover:text-red-500 opacity-0 group-hover/sub:opacity-100 transition-opacity"><XIcon /></button>
                             </div>
                          ))}
                          <div className={`flex gap-2 items-center mt-4 pt-3 border-t border-dashed ${isDark ? 'border-[#333]' : 'border-gray-100'}`}>
                             <input value={newSub.catId === cat.id ? newSub.en : ''} onChange={e => setNewSub({...newSub, catId: cat.id, en: e.target.value})} placeholder="e.g. Scrambled" className={`w-full text-xs bg-transparent border-b p-1 outline-none ${textMuted} border-gray-300 focus:border-[#3c3728]`} dir={lang === 'ar' ? 'rtl' : 'ltr'} />
                             <input value={newSub.catId === cat.id ? newSub.ar : ''} onChange={e => setNewSub({...newSub, catId: cat.id, ar: e.target.value})} placeholder="مثال: مخفوق" className={`w-full text-xs bg-transparent border-b p-1 outline-none ${textMuted} border-gray-300 focus:border-[#3c3728]`} dir={lang === 'ar' ? 'rtl' : 'ltr'} />
                             <button onClick={() => handleAddSub(cat.id)} className="text-[#3c3728] hover:scale-110 transition-transform"><PlusIcon /></button>
                          </div>
                       </div>
                    </div>
                 ))}
